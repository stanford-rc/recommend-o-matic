  ...
  template: `
	...
	<table class="table table-hover">
	  ...
	  <tbody>
		...
		<div class="person-name"><a class="link" :href="resource.url" target="_blank">{{ resource.name }} <span style="font-size:10px">🔗️</span></a>
	      ...
	      <td v-for="(items, key, index) in resource" v-if="key.includes('question_')" :colspan='colspan' :class="selected_class + ' text-white cell row-resource-' + resource.unique_id" :id="'cell-question-' + key + '-resource-' + resource.unique_id">
                  <div class="issue-details"><div class="details" v-for="item in items"></div>
                  </div>
              </td>
	    </tr>
	  </tbody>
